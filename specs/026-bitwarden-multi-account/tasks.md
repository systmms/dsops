...
- [ ] T008 Implement `func (bw *BitwardenProvider) bwEnv() []string` in `internal/providers/bitwarden.go` and update `run(...)` to call `ExecuteWithEnv` when both `bwEnv()` returns a non-empty slice and the executor satisfies `pkgexec.EnvCommandExecutor`; emit a one-time warning at construction if `appDataDir` is set but the executor does not satisfy the interface. T007 must pass; pre-existing Bitwarden tests must still pass.
